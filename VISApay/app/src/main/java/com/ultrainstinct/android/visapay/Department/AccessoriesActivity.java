    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnSession.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                SharedPreferences sharedPreferences = getBaseContext().getSharedPreferences("Accessories", Context.MODE_PRIVATE);

                if(sharedPreferences == null || sharedPreferences.getInt("Accessories",Context.MODE_PRIVATE) == 0){
                    Toast.makeText(getBaseContext(),"Welcome! Incrementing headcount",Toast.LENGTH_SHORT).show();
                    SharedPreferences sharedPref = getBaseContext().getSharedPreferences("Accessories",Context.MODE_PRIVATE);
                    SharedPreferences.Editor editor =sharedPref.edit();
                    editor.putInt("Accessories",1);
                    editor.commit();
